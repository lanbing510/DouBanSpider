...
# if your version of python 2,you should use the following comment
# reload(sys)
...
def book_spider(book_tag):
    ...
    while(True):
        ...
        if list_soup is None and try_times < 200:
            ...
            break
            # Break when no informatoin got after 200 times requesting
...
def do_spider(my_book_tag_lists):
    for book_tag in my_book_tag_lists:
        my_book_lists = book_spider(book_tag)
        my_book_lists = sorted(my_book_lists, key=lambda x: x[1], reverse=True)
        book_lists.append(my_book_lists)
    return book_lists
...
def print_book_lists_excel(my_book_lists, my_book_tag_lists):
    ...
    for i in range (len (my_book_tag_lists)):
        ws.append (wb.create_sheet (title=my_book_tag_lists[i].decode ()))  # utf8->unicode
    for i in range (len (my_book_tag_lists)):
    ...
    save_path = 'book_list'
    for i in range (len (my_book_tag_lists)):
        save_path += ('-' + my_book_tag_lists[i].decode())
    save_path += '.xlsx'
    wb.save(save_path)
